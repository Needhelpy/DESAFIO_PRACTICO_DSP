    ...
    public class Conexion
    {
        ...
        public List<Datos> CargarDatos(int categoria = 1, int municipio = 1, int empresa = 1)
        {
            ...
            SQL = "SELECT categorias.sNombreCategoria, ofertas.nIdOferta, " +
                "ofertas.sTituloOferta, nTipoContrato = CASE ofertas.nTipoContrato WHEN 1 THEN " +
                "'Permanente' WHEN 2 THEN 'Por proyecto' END, nTipoJornada = CASE ofertas.nTipoJornada WHEN 1 " +
                "THEN 'Tiempo Completo' WHEN 2 THEN 'Por horas' END, ofertas.fSalario, ofertas.dFechaPublicacion, " +
                "empresas.sNombreEmpresa, municipios.sNombreMunicipio FROM municipios INNER JOIN ofertas ON " +
                "municipios.nIdMunicipio = ofertas.nIdLocalidad INNER JOIN empresas ON ofertas.nIdEmpresa = " +
                "empresas.nIdEmpresa INNER JOIN categorias ON ofertas.nIdCategoria = categorias.nIdCategoria " +
                "WHERE categorias.nIdCategoria = @nIdCategoria AND municipios.nIdMunicipio = @nIdMunicipio AND " +
